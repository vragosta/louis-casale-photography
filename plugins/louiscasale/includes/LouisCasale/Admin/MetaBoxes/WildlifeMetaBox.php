...
class WildlifeMetaBox {
	 ...
	 */
	function register() {
		add_action( 'add_meta_boxes', array( $this, 'louiscasale_wildlife_metaboxes' ) );
		add_action( 'save_post', array( $this, 'louiscasale_wildlife_save_data' ) );
	}
	 ...
	 * @return void
	 */
	function louiscasale_wildlife_metaboxes() {
		add_meta_box(
			'configuration',
			__( 'Configuration', 'louiscasale_com' ),
			array( $this, 'louiscasale_wildlife_callback' ),
			WILDLIFE_POST_TYPE
		);
	}
	 ...
	 * @return void
	 */
	function louiscasale_wildlife_callback( $post ) {
		# Add a nonce field so we can check for it later.
		wp_nonce_field( 'louiscasale_wildlife_save_data', 'louiscasale_wildlife_nonce' );

		# Transfer $key to _$key.
		$this->transfer_key( $post->ID, 'featured' );

		# Call WildlifeFinder class methods.
		$finder = new WildlifeFinder( $post->ID );
		...
		<tr>
			<td>
				<label for="_featured"><?php echo esc_html( __( 'Feature Wildlife', 'louiscasale_com' ) ); ?></label>
			</td>
			<td>
		...
		<tr>
			<td>
				<label for="_favorited"><?php echo esc_html( __( 'Favorite Wildlife', 'louiscasale_com' ) ); ?></label>
			</td>
			<td>
	 ...
	 * @return void
	 */
	function louiscasale_wildlife_save_data( $post_id ) {
		/**
		 * We need to verify this came from our screen and with proper authorization,
		 * because the save_post action can be triggered at other times.
		 */
		if ( ! isset( $_POST['louiscasale_wildlife_nonce'] ) ) {
			return;
		}

		# Verify that the nonce is valid.
		if ( ! wp_verify_nonce( $_POST['louiscasale_wildlife_nonce'], 'louiscasale_wildlife_save_data' ) ) {
			return;
		}
